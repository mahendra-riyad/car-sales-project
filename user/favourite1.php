...
<div class="column middle">
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      var v=  $(this).attr("id");
      $.post("favourite2.php",{car_code:v},function(data){
          $("#"+v).fadeOut(1000);
        });
...
<p style="font-size: 20px; letter-spacing: 2px; word-spacing: 4px; color:#fff;padding: 20px;">Favourite Cars :-</p>
<?php include("db.php");
$pp=mysqli_query($con,"select * from favourite where user_code='$user_code'");
while($p=mysqli_fetch_array($pp)){
  
    $rs=mysqli_query($con,"select * from car_short_details where car_code='$p[0]'");
    while($r=mysqli_fetch_array($rs))
    {
  ?> 
  
   <div class="back"><div class="row" id="<?=$r[1]?>" style="padding:10px; background-color:#101010; margin-bottom:10px; margin-left:0px; margin-right:0px;">
    <span class="glyphicon glyphicon-remove" id="<?=$r[1]?>" style="float: right; font-size: 20px; pointer:cursor;"></span>
...
<?php
 ...
 ?>
  ...
  </div></div><div class="col-sm-3" style="padding-left:30px;"><b style="font-size:24px; color:#FFFF9D;">$ <?=$r[5]?></b><br>
  ...
  <a href="view_details.php?id=<?=$r[1]?>"><button class="view-details">View Details</button></a>
  </div></div></div>
  <?php
    } }?>
</div></div>
